...
$whereMotorista = "m.status='Ativo'";
$whereViatura = "v.estado='Disponível'";
if (is_gestor_zona()) {
  $zid = (int)zona_id_sessao();
  $whereMotorista .= " AND m.zona_operacional_id = $zid";
  $whereViatura .= " AND v.zona_operacional_id = $zid";
}

$motoristas = [];
$resM = mysqli_query($ligacao, "
  SELECT m.id, m.nome, m.nif FROM motoristas m
  WHERE $whereMotorista
    AND m.id NOT IN (SELECT motorista_id FROM atribuicoes WHERE estado='aberta' AND motorista_id IS NOT NULL)
  ORDER BY m.nome ASC
");
if ($resM) {
...
$resV = mysqli_query($ligacao, "
  SELECT v.id, v.matricula, v.marca_modelo, v.quilometragem FROM viaturas v
  WHERE $whereViatura
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (is_gestor_zona()) {
    $zid = (int)zona_id_sessao();
    if ($viatura_id > 0) {
      $chkV = mysqli_query($ligacao, "SELECT zona_operacional_id FROM viaturas WHERE id = " . $viatura_id);
    ...
    if ($motorista_id > 0) {
      $chkM = mysqli_query($ligacao, "SELECT zona_operacional_id FROM motoristas WHERE id = " . $motorista_id);
      if ($chkM && $rM = mysqli_fetch_assoc($chkM)) {
        if ($rM['zona_operacional_id'] !== null && (int)$rM['zona_operacional_id'] !== $zid) {
          $erros[] = "O motorista selecionado não pertence à sua zona.";
        }
      }
    }
  }
  ...
  if (empty($erros)) {
    ...
    try {
      ...
      // 6. Atualizar quilometragem e recalcular o estado do veículo
      $stmtUpdV = mysqli_prepare($ligacao, "UPDATE viaturas SET quilometragem=? WHERE id=?");
      mysqli_stmt_bind_param($stmtUpdV, "ii", $km_inicio_int, $viatura_id);
      mysqli_stmt_execute($stmtUpdV);
      mysqli_stmt_close($stmtUpdV);

      recalcular_estado_viatura($ligacao, $viatura_id);
