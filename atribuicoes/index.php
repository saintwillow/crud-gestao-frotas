...
$r1 = mysqli_query($ligacao, "SELECT COUNT(*) AS t FROM atribuicoes WHERE estado = 'aberta'");
if ($r1) $kpiAbertas = (int)(mysqli_fetch_assoc($r1)['t'] ?? 0);

$r2 = mysqli_query($ligacao, "SELECT COUNT(*) AS t FROM atribuicoes WHERE estado = 'encerrada'");
if ($r2) $kpiEncerradas = (int)(mysqli_fetch_assoc($r2)['t'] ?? 0);

$r3 = mysqli_query($ligacao, "SELECT COUNT(*) AS t FROM motoristas m WHERE m.status = 'Ativo' AND NOT EXISTS (SELECT 1 FROM atribuicoes a WHERE a.motorista_id = m.id AND a.estado = 'aberta')");
if ($r3) $kpiMotoristasSem = (int)(mysqli_fetch_assoc($r3)['t'] ?? 0);

$r4 = mysqli_query($ligacao, "SELECT COUNT(*) AS t FROM viaturas v WHERE v.estado = 'Disponível'");
    ...
    <div class="d-flex align-items-center gap-2 flex-wrap">
      <?php if (isset($_GET['msg'])): ?>
        <?php if ($_GET['msg'] === 'criada'): ?>
          <div class="alert alert-success py-2 px-3 mb-0">Atribuição criada com sucesso!</div>
        <?php elseif ($_GET['msg'] === 'encerrada'): ?>
          <div class="alert alert-success py-2 px-3 mb-0">Atribuição encerrada com sucesso!</div>
        <?php endif; ?>
      <?php endif; ?>
