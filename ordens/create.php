...
// Carregar infraestruturas, viaturas e motoristas respeitando os limites da zona operacional
$whereInfra = "ativo=1";
$whereViatura = "1=1";
...
$motoristas = [];
$resM = mysqli_query($ligacao, "SELECT id, nome, nif FROM motoristas WHERE $whereMotorista ORDER BY nome ASC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Validação de segurança ao salvar
  if (is_gestor_zona()) {
    $zid = (int)zona_id_sessao();
    if ($viatura_id_val !== null) {
      $chkV = mysqli_query($ligacao, "SELECT zona_operacional_id FROM viaturas WHERE id = $viatura_id_val");
      if ($chkV && $rV = mysqli_fetch_assoc($chkV)) {
        if ((int)$rV['zona_operacional_id'] !== $zid) $erros[] = "A viatura selecionada não pertence à sua zona.";
      }
    }
    if ($motorista_id_val !== null) {
      $chkM = mysqli_query($ligacao, "SELECT zona_operacional_id FROM motoristas WHERE id = $motorista_id_val");
