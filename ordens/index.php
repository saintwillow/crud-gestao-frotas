...
$filtro_zona_kpi = "";
if ($filtro_zona_v !== "") {
  $filtro_zona_kpi = " AND viatura_id IN (SELECT id FROM viaturas v WHERE 1=1 {$filtro_zona_v})";
}

$r1 = mysqli_query($ligacao, "SELECT COUNT(*) AS t FROM ordens_servico WHERE 1=1{$filtro_zona_kpi}");
if ($r1) $kpiTotal = (int)(mysqli_fetch_assoc($r1)['t'] ?? 0);

$r2 = mysqli_query($ligacao, "SELECT COUNT(*) AS t FROM ordens_servico WHERE estado IN ('atribuida','aceite'){$filtro_zona_kpi}");
if ($r2) $kpiPendentes = (int)(mysqli_fetch_assoc($r2)['t'] ?? 0);

$r3 = mysqli_query($ligacao, "SELECT COUNT(*) AS t FROM ordens_servico WHERE estado IN ('em_deslocacao','em_execucao'){$filtro_zona_kpi}");
if ($r3) $kpiExecucao = (int)(mysqli_fetch_assoc($r3)['t'] ?? 0);

$r4 = mysqli_query($ligacao, "SELECT COUNT(*) AS t FROM ordens_servico WHERE estado = 'concluida'{$filtro_zona_kpi}");
if ($r4) $kpiConcluidas = (int)(mysqli_fetch_assoc($r4)['t'] ?? 0);

$r5 = mysqli_query($ligacao, "SELECT COUNT(*) AS t FROM ordens_servico WHERE estado = 'impedida'{$filtro_zona_kpi}");
